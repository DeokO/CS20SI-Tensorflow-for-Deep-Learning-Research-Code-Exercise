...
with tf.Session() as sess:
    sess.run(init)
    writer = tf.summary.FileWriter('./graphs/notes03/logisticRegression', sess.graph)
    n_batches = int(MNIST.train.num_examples / batch_size) #실제로 1번 epoch에서 돌아갈 배치 횟수

    for i in range(n_epochs): #25회 epoch
        ...
        print('Epoch{} _ loss : {:.4f} | predict :{}'.format(i, ll, pred)) #predict를 print 하는 부분을 생략하는게 속도를 위해서는 좋을 수도 있겠다.

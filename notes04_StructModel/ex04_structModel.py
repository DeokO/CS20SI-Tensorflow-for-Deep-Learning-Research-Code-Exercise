...
SKIP_STEP = 5000        #5000번마다 한번씩 loss를 출력해줌
...
class SkipGramModel:
    ...
    #loss를 정의한다.
    #이 역시 gpu상에 생성하고, name_scope를 'loss'라 이름 붙인다.
...
def train_model(model, batch_gen, num_train_steps):
    ...
    writer = tf.summary.FileWriter('./graphs/notes04/skip-gram_graph/lr' + str(LEARNING_RATE), sess.graph)
    ...
    for index in range(initial_step, initial_step + num_train_steps):
        ...
        if (index+1) % SKIP_STEP == 0:
            ...
            # variable 형태만 작동할 수 있다. variable 타입으로 만들어주자. 이때, final_embed_matrix를 재사용하면 initialize를 학습한 내용으로 할 수 있다.
            ...
            summary_writer = tf.summary.FileWriter('skip-gram_processed')
            ...
            embedding.metadata_path = './graphs/notes04/skip-gram_processed/vocab_1000.tsv'

            # saves a configuration file that TensorBoard will read during startup.
            # 여기에는 embedding_var(단어 임베딩 정보)만을 담는다.
            projector.visualize_embeddings(summary_writer, config)
            saver_embed = tf.train.Saver([embedding_var])
            saver_embed.save(sess, './graphs/notes04/skip-gram_processed/model_processed.ckpt', saver_num)

    saver.save(sess, './graphs/notes04/skip-gram_checkpoints/skipGram_model.ckpt')  # 이름이 제대로 저장되지 않음. 인코딩 문제인가


# 저장용 폴더를 미리 만들어둬야 한다.
# os.makedirs('./graphs/notes04/skip-gram_checkpoints/')
# os.makedirs('./graphs/notes04/skip-gram_processed')
